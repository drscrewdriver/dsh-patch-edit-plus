...
import type { Hunk, PatchLine } from '../parse/types.js'
 ...
 */
export async function verifyPatch(ctx: Context, exec: ToolRunContext, plan: PlannedPatch, cfg: ResolvedConfig): Promise<VerifiedChange[]> {
  ...
  for (const entry of plan.entries) {
    ...
    if (op.kind === 'add') {
      ...
      if (existing !== undefined) throw addExistsError(op.path)
      changes.push({ entry, before: null, after: joinContent(op.lines, op.trailingNewline, false) })
      continue
    }
    if (op.kind === 'delete') {
      const existing = await statFile(ctx, exec, entry.target)
      if (existing === undefined) throw missingFileError(op.path, 'delete')
...
export async function applyPatch(ctx: Context, exec: ToolRunContext, changes: readonly VerifiedChange[], cfg: ResolvedConfig, sandboxPolicy: SandboxExecutionPolicy | undefined): Promise<void> {
...
export function isPureMove(change: VerifiedChange): boolean {
  return change.entry.op.kind === 'update'
    && change.entry.op.hunks.length === 0
    && change.entry.moveToTarget !== undefined
}

function joinContent(lines: readonly string[], trailingNewline: boolean, crlf: boolean): string {
 ...
 */
export function applyHunksToContent(path: string, before: string, hunks: readonly Hunk[]): string {
...
function resolveTrailingNewline(lastHunk: Hunk | undefined, reachesEnd: boolean, beforeEndsWithNewline: boolean): boolean {
  if (lastHunk === undefined || !reachesEnd) return beforeEndsWithNewline
  const tail = newSideTail(lastHunk)
  if (tail === 'context') return beforeEndsWithNewline
...
function newSideTail(hunk: Hunk): 'context' | 'add' {
  for (let i = hunk.lines.length - 1; i >= 0; i -= 1) {
    const kind = hunk.lines[i].kind
    if (kind === 'context' || kind === 'add') return kind
  }
  return 'context'
}

/** Locate one hunk with the header-line hint, falling back to the plain cursor. */
function locate(fileLines: readonly string[], hunk: Hunk, cursor: number, path: string, hunkIndex: number): number {
  ...
  throw new PatchError('VALIDATION', mismatchMessage({
    ...
    expected: hunk.lines.filter((line: PatchLine) => line.kind !== 'add').map(line => line.text),
