...
export function findSequence(
  ...
  expected: readonly PatchLine[],
  from: number,
  opts: { endOfFile?: boolean } = {},
): number | null {
  const sequence = expected.filter(line => line.kind !== 'add').map(line => line.text)
  if (sequence.length === 0) return from <= fileLines.length ? from : null
  ...
  for (const equal of comparators) {
    for (let i = start; i <= lastStart; i += 1) {
      if (matchesAt(fileLines, sequence, i, equal)) return i
    }
  }
  return null
}

function matchesAt(fileLines: readonly string[], sequence: readonly string[], at: number, equal: (a: string, b: string) => boolean): boolean {
  for (let j = 0; j < sequence.length; j += 1) {
    if (!equal(fileLines[at + j], sequence[j])) return false
  }
  return true
}
 ...
 */
export function consumedLines(hunk: { lines: readonly PatchLine[] }): number {
