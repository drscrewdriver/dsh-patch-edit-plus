 ...
 */
export function registerApplyPatchTool(ctx: Context, cfg: ResolvedConfig, logger?: LoggerFace): (() => void) | null {
  const log: LoggerFace = logger ?? resolveLogger(ctx)
  ...
  if (tools.get(resolvedName) !== undefined) {
    ...
    for (let attempt = 1; attempt <= MAX_RENAME_ATTEMPTS && found === null; attempt += 1) {
      ...
      if (tools.get(candidate) === undefined) found = candidate
    }
    if (found === null) {
  ...
  try {
    ...
    if (cfg.conflictPolicy === 'fail') throw error
...
function resolveLogger(ctx: Context): LoggerFace {
  const candidate = (ctx as unknown as { logger?: LoggerFace }).logger
