...
interface FileSection {
  ...
  hunks: Hunk[]
  /** 1-based line number of the section start, for error messages. */
  startLine: number
}
...
export function parseUnifiedDiff(patch: string): ParsedPatch {
  ...
  if (lines.length === 0) throw unrecognizedPatchError()
  ...
  while (index < lines.length) {
    ...
    if (line.startsWith('+++ ')) {
      if (current === null) throw parseError(lineNo, '+++ header without a preceding --- header')
      const token = line.slice(4).trim()
    ...
    if (line.startsWith('new file mode')) { if (current === null) throw parseError(lineNo, 'mode line outside a file section'); current.newFile = true; index += 1; continue }
    if (line.startsWith('deleted file mode')) { if (current === null) throw parseError(lineNo, 'mode line outside a file section'); current.deletedFile = true; index += 1; continue }
    if (line.startsWith('rename from ')) { if (current === null) throw parseError(lineNo, 'rename line outside a file section'); current.renameFrom = stripPrefix(line.slice(12).trim()); index += 1; continue }
    if (line.startsWith('rename to ')) { if (current === null) throw parseError(lineNo, 'rename line outside a file section'); current.renameTo = stripPrefix(line.slice(10).trim()); index += 1; continue }
    if (line.startsWith('old mode ') || line.startsWith('new mode ') || line.startsWith('similarity index ')
      ...
      if (current === null) throw parseError(lineNo, `"${line.slice(0, 24)}" outside a file section`)
      index += 1
      continue
    }
    ...
    if (line.startsWith('@@')) {
      if (current === null) throw parseError(lineNo, 'hunk header outside a file section')
    ...
    if (current === null) throw unrecognizedPatchError()
    throw parseError(lineNo, `unexpected line "${line.slice(0, 48)}"`)
  }

  if (sections.length === 0) throw unrecognizedPatchError()
...
function parseHunkHeader(line: string, lineNo: number): HunkHeaderInfo {
  const match = HUNK_HEADER.exec(line)
  if (match === null) throw parseError(lineNo, `malformed hunk header "${line.slice(0, 48)}"`)
...
function parseHunkBody(lines: string[], start: number, header: HunkHeaderInfo): HunkBodyResult {
  ...
  while (index < lines.length) {
    ...
    if (oldSeen >= header.oldCount && newSeen >= header.newCount
      && (line.startsWith('diff --git ') || line.startsWith('@@') || line.startsWith('--- ') || line.startsWith('*** '))) break
    if (line.startsWith('\\')) {
      ...
      if (result.length === 0) throw parseError(index + 1, '\\ "no newline" marker before any hunk line')
      const previous = result.at(-1)
      if (previous?.kind === 'delete') oldNoNewline = true
      else if (previous?.kind === 'add') newNoNewline = true
...
function assertPlainPath(path: string | null, lineNo: number): string {
  if (path === null) throw parseError(lineNo, 'file section without a usable path')
  return path
}

/** Convert one parsed file section into an IR operation. */
function toOp(section: FileSection): PatchOp {
  ...
  if (section.newFile || (section.oldPath === null && section.newPath !== null)) {
    ...
    for (const hunk of section.hunks) {
      for (const line of hunk.lines) {
        if (line.kind === 'add') addLines.push(line.text)
        else if (line.kind === 'delete') throw parseError(headerLine, `new-file section for "${path}" contains delete lines`)
      }
    }
