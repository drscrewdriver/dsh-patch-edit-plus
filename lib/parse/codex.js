...
export function parseCodexPatch(patch) {
    ...
    if (lines.at(-1) !== END)
        throw parseError(lines.length, `patch must end with "${END}"`);
    const ops = [];
    ...
    while (index < lines.length) {
        ...
        if (line.startsWith(DELETE_FILE)) {
            ...
            if (path === '')
                throw parseError(lineNo, `${DELETE_FILE}requires a file path`);
    ...
    if (ops.length === 0)
        throw parseError(1, 'patch contains no file operations');
...
function parseAddFile(lines, start) {
    const path = lines[start].slice(ADD_FILE.length).trim();
    if (path === '')
        throw parseError(start + 1, `${ADD_FILE}requires a file path`);
    const content = [];
    let index = start + 1;
    while (index < lines.length) {
        const line = lines[index];
...
function parseUpdateFile(lines, start) {
    const path = lines[start].slice(UPDATE_FILE.length).trim();
    if (path === '')
        throw parseError(start + 1, `${UPDATE_FILE}requires a file path`);
    let index = start + 1;
    let moveTo;
    if (lines[index]?.startsWith(MOVE_TO)) {
        const target = lines[index].slice(MOVE_TO.length).trim();
        if (target === '')
            throw parseError(index + 1, `${MOVE_TO}requires a file path`);
        moveTo = target;
        index += 1;
    }
    const hunks = [];
    while (index < lines.length) {
        ...
        if (line === END)
            break;
        if (line.startsWith(UPDATE_FILE) || line.startsWith(ADD_FILE) || line.startsWith(DELETE_FILE))
            break;
        if (!line.startsWith('@@')) {
    ...
    if (hunks.length === 0) {
        ...
        if (moveTo === undefined)
            throw parseError(start + 1, `update of "${path}" contains no hunks`);
    }
    return { op: { kind: 'update', path, moveTo, hunks }, nextIndex: index };
}
function parseCodexHunk(lines, start, path) {
    const result = [];
    let endOfFile = false;
    let index = start;
    while (index < lines.length) {
        ...
        if (marker === ' ')
            ...
            throw parseError(index + 1, `hunk lines of "${path}" must start with " ", "-", or "+"; got "${line.slice(0, 48)}"`);
        index += 1;
    }
    if (result.length === 0)
        throw parseError(start + 1, `hunk of "${path}" is empty`);
